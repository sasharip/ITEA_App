...
/*
* 0 Красота кода страдает. Очень.
* 1 Посреди кода не писать findViewById. Все вью должны быть "найдены" в самом начале onCreate
* 2 Немного боьше некой "инкапсуляции" - разделить на методы которые делают нидентичные задачи.
*   Бородатый onCreate не айс
* 3 262 строка НЕ ИСПОЛЬЗОВАТЬ
* 4 Все обекты уже обявлены, зачем из переобявлять ???
* 5 setOnClickListener незачем вешать на чекБокс. setOnClickListener словит нажатие на ЧБ а не то была ли поставлена
*   галочка
* 6 Все проверки нужно делать по нажатию на определенную кнопку а не в onCreate.
*   onCreate срабаотывает при запуске активити,  а при запуске все поля пусты, все чекбоксы пусты и радиокнопки тоже пусты
*   Следовательно проверки нам ничего не дадут
* */
